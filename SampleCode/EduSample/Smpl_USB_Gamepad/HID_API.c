//
// HID_API : using Joystick and/or Keypad 3x3
// 	     This API support Gamepad, Digitizer, Mouse, Keyboard
//
// Joystick to Nu-LB-NUC140
// GND : to Gnd
// +5V : to Vcc
// VRx : to ADC6
// VRy : to ADC7
...
//-------------------------------------------------------------------------------------------------
	...
	const uint8_t g_HID_au8GamepadReportDescriptor[] =
	{
...
#define HID_GAMEPAD_REPORT_DESCRIPTOR_SIZE \
sizeof (g_HID_au8GamepadReportDescriptor) / sizeof(g_HID_au8GamepadReportDescriptor[0])
const uint32_t g_HID_u32GamepadReportDescriptorSize = HID_GAMEPAD_REPORT_DESCRIPTOR_SIZE;

#define HID_REPORT_DESCRIPTOR_SIZE HID_GAMEPAD_REPORT_DESCRIPTOR_SIZE

uint8_t g_au8GamepadReport[HID_REPORT_SIZE];
const uint32_t g_u32GamepadReportSize = sizeof(g_au8GamepadReport) / sizeof(g_au8GamepadReport[0]);
#endif
//-------------------------------------------------------------------------------------------------
...
// Report Descriptor - keyboard 101keys
const uint8_t g_HID_au8KeyboardReportDescriptor[] = {
      ...
      0x95, 0x01, //    Report Count(1)
      0x75, 0x08, //    Report Size (8)
      0x81, 0x01, //    Input (Constatnt)                ;Reserved byte
...
// Report Descriptor - Multi-touch digitizer
...
void HID_Init(void)
{
  ...
  /* Set the HID report descriptor */
	HID_SetReportDescriptor(g_HID_au8GamepadReportDescriptor, g_HID_u32GamepadReportDescriptorSize);
  /* Set the HID report buffer */
	HID_SetReportBuf(g_au8GamepadReport, g_u32GamepadReportSize);		
#elif(HID_FUNCTION == HID_MOUSE)
  /* Set the HID report descriptor */
...
void HID_SetInReport()
{
  ...
  buf = g_HID_sDevice.pu8Report;
	...
	if(keyin!=0)
	{
		Button_Value = 1<<(keyin-1);
		sprintf(LCD_Value, "Key%2d   ", keyin);
		print_Line(1, LCD_Value);
	}
	else
	{
		Button_Value = 0;
		print_Line(1, "No press");
	}
	
	//Update Joys ADC Value	
	while(ADC->ADSR.ADF==0);
	ADC->ADSR.ADF = 1;
	VRx = (ADC->ADDR[6].RSLT>>4) - 128;		//VRx: ADC6(GPA6)
	VRy = (ADC->ADDR[7].RSLT>>4) - 128;		//VRy: ADC7(GPA7)
	ADC->ADCR.ADST = 1;
	sprintf(LCD_Value, "X:%4d, Y:%4d", VRx, VRy);
	print_Line(2, LCD_Value);
     			
	//Update Joys Key Value
	DrvGPIO_SetBit(E_GPB, 15);					//Set GPB15 to high
	if(DrvGPIO_GetBit(E_GPB, 15)==0)			//Read GPB15 Value
	{
		Button_Value |= 1<<(15-1);
		print_Line(3, "JoysKey:Press   ");
	}
	else
	{
		Button_Value &= ~(1<<(15-1));
		print_Line(3, "JoysKey:No Press");
	}

	//Update HID Report
	buf[0] = Button_Value;
	buf[1] = Button_Value>>8;
	buf[2] = VRx;
	buf[3] = VRy;
	buf[4] = 0;
	buf[5] = 0;
...
#elif(HID_FUNCTION == HID_MOUSE)
/* This function is used to prepare IN report. 
The g_HID_sDevice.u32ReportSize will be reset when it has been done. */
void HID_SetInReport()
{
    uint8_t *buf;
	  uint8_t keyin;
	  uint8_t button, movX, movY;
    /* To check if previous report data is processed or not */
    if(g_HID_sDevice.isReportReady) return;

    buf = g_HID_sDevice.pu8Report;

		keyin=ScanKey();
		if (keyin==5) button=0x01; else button =0x00;		
		switch(keyin) {
			case 1: movX=-1; movY=-1; print_Line(2, "Move Left-Up   "); break;
			case 2: movX= 0; movY=-1; print_Line(2, "Move Up        "); break;
			case 3: movX= 1; movY=-1; print_Line(2, "Move Right-Up  "); break;
			case 4: movX=-1; movY= 0; print_Line(2, "Move Left      "); break;
			case 5: movX= 0; movY= 0; print_Line(2, "Button         "); break;
			case 6: movX= 1; movY= 0; print_Line(2, "Move Right     "); break;			
			case 7: movX=-1; movY= 1; print_Line(2, "Move Left-Down "); break;
			case 8: movX= 0; movY= 1; print_Line(2, "Move Down      "); break;
			case 9: movX= 1; movY= 1; print_Line(2, "Move Right-Down"); break;	
			default:movX= 0; movY= 0; print_Line(2, "No Key Press   "); break;
		}
...
#else // HID_FUNCTION = KEYBOARD
void HID_SetInReport()
{
    uint8_t *buf;
    uint8_t keyin;

    if(g_HID_sDevice.isReportReady) return; // check if previous report done
	
    buf = g_HID_sDevice.pu8Report; // update new report data
            
    keyin = ScanKey();   // scan keypad 3x3
    buf[2] = 0x03+keyin; // press 1~9 output a~i 
    g_HID_sDevice.isReportReady = 1;
    DrvUSB_DataIn(INT_IN_EP_NUM, g_HID_sDevice.pu8Report, g_HID_sDevice.u32ReportSize);
}
#endif

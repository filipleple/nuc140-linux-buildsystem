//
// Smpl_ADC_PWM : 
// VR1 change ADC7 value
// then set as PWM0 HiTime to control LED (GPA12)
// The red LED is low-active (PWM output Hi = LED off)
//
#include <stdio.h>																											 
#include "NUC1xx.h"
#include "GPIO.h"
...
int32_t main (void)
{
  ...
  print_Line(0,"Smpl_ADC_PWM");	
	while(1)
	{
		...
		sprintf(adc_value,"ADC7 = %4d",ADC->ADDR[7].RSLT);
		print_Line(1, adc_value);
	}
}
